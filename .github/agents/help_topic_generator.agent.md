...
description: Generates or updates the currently opened API help topic using GitHub PRs and specifications.
...
- **GitHub Pull Request URLs**: The URLs of PRs that contains the API changes.
- **Specifications**: _Optional_. Markdown or PDF files that provide additional details about the API changes. Basic specifications may be included in PR descriptions or comments.  
  If specifications are provided, treat them as the "source of truth" over PR descriptions if conflicts arise.
...
4. Analyze the diff, description, comments, and any attached specification files to extract relevant information about the API changes.  
   If a fact is not supported by PR diffs, descriptions, comments, or specification files, do not infer or invent details. Ask the user for clarification instead.
