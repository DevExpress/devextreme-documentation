...
---
If this property is enabled, {WidgetName} hides columns that do not fit in the component container. {WidgetName} displays a [command column]() that allows users to view hidden column data.
...
Note the following **columnHidingEnabled** specifics:
...
- If {WidgetName} hides an AI column, users cannot access the [AI header menu](/showHeaderMenu) to regenerate data or update the prompt.
