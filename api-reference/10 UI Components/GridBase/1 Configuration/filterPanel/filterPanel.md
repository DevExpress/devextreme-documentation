...
[note] If filter panel is visible and at least one column includes **headerFilter**.[dataSource]({basewidgetpath}/Configuration/columns/headerFilter/#dataSource) or **lookup**.[dataSource]({basewidgetpath}/Configuration/columns/lookup/#dataSource), disable the [syncLookupFilterValues]({basewidgetpath}/Configuration/#syncLookupFilterValues) property. Otherwise, the filter panel may not display data correctly.
