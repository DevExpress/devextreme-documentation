...
Alternatively, error bar values can be calculated according to an algorithm. In this case, choose the required algorithm using the **type** option, and specify the value to be used in calculation using the **value** property.

[note]Error bars are available for axes of the numeric [type](/api-reference/20%20Data%20Visualization%20Widgets/dxChart/1%20Configuration/argumentAxis/type.md '/Documentation/ApiReference/UI_Components/dxChart/Configuration/argumentAxis/#type') only.
