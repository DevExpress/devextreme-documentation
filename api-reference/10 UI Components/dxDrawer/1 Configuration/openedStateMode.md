...
- *"overlap"*       
The drawer overlaps the view. If you assign *"overlap"* to the **openedStateMode** property, the Drawer uses the [Popup](/Documentation/ApiReference/UI_Components/dxPopup/) component to render a side menu. A z-index of all overlay UI components (Popup, Popover, etc.) located on a page is calculated when a component is created and it takes into account z-indexes of existing overlays. As a result, a newly opened Popup (the Drawer's panel) is displayed at the top. If you need to display a different window at the top, call the [focus()](/Documentation/ApiReference/UI_Components/dxPopup/Methods/#focus) method for the Popup after the **openedStateMode** property is changed.
