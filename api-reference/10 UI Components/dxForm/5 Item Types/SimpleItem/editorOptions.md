...
[note] If you set the `id` and `name` attributes using the [inputAttr](/api-reference/10%20UI%20Widgets/dxTextEditor/1%20Configuration/inputAttr.md '/Documentation/ApiReference/UI_Components/dxTextBox/Configuration/#inputAttr') option, they will be overwritten. The **Form** generates these attributes automatically and uses them for addressing the DOM elements.
