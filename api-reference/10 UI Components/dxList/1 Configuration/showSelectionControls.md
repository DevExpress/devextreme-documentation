...
[note] If this setting is enabled, [displayExpr](/Documentation/ApiReference/UI_Components/dxList/Configuration/#displayExpr) is not defined, and no [text](/Documentation/ApiReference/UI_Components/dxList/Configuration/items/#text) field exists for an item, List cannot identify the display data and specifies aria-label as `List item content`. To set another aria-label, define **displayExpr**.
