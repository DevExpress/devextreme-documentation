---
id: dxLoadPanel.Options.position
acceptValues: 'bottom' | 'center' | 'left' | 'left bottom' | 'left top' | 'right' | 'right bottom' | 'right top' | 'top'
type: String | PositionConfig | function()
---

If you do not specify the [container]({basewidgetpath}/Configuration/#container) property and set the **position**.[of](/Documentation/ApiReference/Common/Object_Structures/PositionConfig/#of) to the particular element, the [shading]({basewidgetpath}/Configuration/#shading) applies to this element. The {WidgetName} calculates its size relatively to this element.