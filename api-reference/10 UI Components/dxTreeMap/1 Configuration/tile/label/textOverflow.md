---
id: dxTreeMap.Options.tile.label.textOverflow
acceptValues: 'ellipsis' | 'hide' | 'none'
type: String
default: 'ellipsis'
---
---
##### shortDescription
Specifies what to do with labels that overflow their tiles after applying [wordWrap](/api-reference/20%20Data%20Visualization%20Widgets/dxTreeMap/1%20Configuration/tile/label/wordWrap.md '/Documentation/ApiReference/UI_Components/dxTreeMap/Configuration/tile/label/#wordWrap'): hide, truncate them and display an ellipsis, or do nothing.

---
#include common-ref-enum with {
    enum: "`VizTextOverflow`",
    values: "`Ellipsis`, `Hide`, and `None`"
}