---
id: dxTreeView.Options.rootValue
type: any
default: 0
---
---
##### shortDescription
Specifies the root key.

---

Assign an item's [parentId](/api-reference/_hidden/dxTreeViewItem/parentId.md '/Documentation/ApiReference/UI_Components/dxTreeView/Configuration/items/#parentId') to the value of this property to specify a root item.

[note] To ensure the component displays data correctly, specify a **rootValue** that does not match **items[]**.[id](/api-reference/_hidden/dxTreeViewItem/id.md '/Documentation/ApiReference/UI_Components/dxTreeView/Configuration/items/#id') properties.