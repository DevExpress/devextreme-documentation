...
A card shape is designed to contain an image. The [defaultImageUrl](/Documentation/ApiReference/UI_Widgets/dxDiagram/Configuration/customShapes/#defaultImageUrl) property specifies the default image that is displayed in the card. Set the **allowEditImage** property to `true` to allow users to change the image from a context menu.
