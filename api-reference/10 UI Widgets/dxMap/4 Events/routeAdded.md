---
type: eventType
---
---
##### shortDescription
Fires after a route is added to the map.

##### param(e): object
Provides function parameters.

##### field(e.component): object
Provides access to the widget's instance.

##### field(e.element): jQuery
An HTML element of the widget.

##### field(e.model): object
Provides access to the data that is available for binding against the element. Available only in the Knockout approach.

##### field(e.options): object
Returns the object that defines the currently added route in the "routes" array.

##### field(e.originalRoute): object
Returns an original route object used by the current map provider (only for 'google' and 'bing' providers).

---
Instead, you can use the [onRouteAdded](/api-reference/10%20UI%20Widgets/dxMap/1%20Configuration/onRouteAdded.md '/Documentation/ApiReference/UI_Widgets/dxMap/Configuration/#onRouteAdded') option to handle the event.

#####See Also#####
#include common-link-handleevents