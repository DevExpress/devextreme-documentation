    ...
    clickHandler: function (e) {
        if (flag === false) {
            myViewModel.panoramaInstance.on(
                "itemClick", function (e) { DevExpress.ui.notify("The \"" + e.itemData.header + "\" item is clicked", "success", 1500); }
            );
            e.component.option("text", "Unsubscribe from ItemClick event");
            flag = true;
        } else {
            myViewModel.panoramaInstance.off("itemClick");
...
myApp.controller("demoController", function ($scope) {
    ...
    $scope.clickHandler = function (e) {
        if (flag === false) {
            ...
            $scope.panoramaInstance.off("itemClick");
            e.component.option("text", "Subscribe to ItemClick event");
            flag = false;
        }
    }
...
$("#myButton").dxButton({
    ...
    onClick: function (e) {
        if (flag === false) {
            ...
            $("#myPanorama").dxPanorama("instance").off("itemClick");
            e.component.option("text", "Subscribe to ItemClick event");
            flag = false;
        }
    }
