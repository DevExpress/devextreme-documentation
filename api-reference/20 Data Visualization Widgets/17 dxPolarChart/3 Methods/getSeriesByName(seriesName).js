...
$(function () {
    ...
    $('#getMaximumValueButton').click(function () {
        var searchedSeriesName = $('#seriesNameTextBox').val();
        var chart = $('#chartContainer').dxPolarChart('instance');
        var series = chart.getSeriesByName(searchedSeriesName);
        if (series) {
            var valueArray = [];
            ...
            $('#selectionSpan').html('Maximum value in the &#34;' + series.name +
                '&#34; series: ' + valueArray.max());
        } else {
            $('#selectionSpan').html('The desired series name is not found');
        }
    });
});