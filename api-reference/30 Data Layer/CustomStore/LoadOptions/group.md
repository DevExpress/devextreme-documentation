...
Defines whether to return the group's items or `null`. For grid-like components (DataGrid, TreeList, and so on), this field always returns **true** for all groups except the last one. DevExtreme components accepts the `unspecified` value of the **isExpanded** field as **true**.
