...
A function that returns one of the above.
    ...
    When the component is initialized, it calls the **dataSource** function with an empty **options** object. To avoid execution errors, check the truthiness of **options** properties before accessing them. The following code snippet demonstrates how you can check the truthiness of **options**.**data** before using the property in a filter expression:
