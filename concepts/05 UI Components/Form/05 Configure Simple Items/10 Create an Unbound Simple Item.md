...
In the following example, the `order` item contains the [DataGrid](/concepts/05%20UI%20Components/DataGrid/00%20Getting%20Started%20with%20DataGrid '/Documentation/Guide/UI_Components/DataGrid/Getting_Started_with_DataGrid/') UI component. This UI component is shown only when the [CheckBox](/concepts/05%20UI%20Components/CheckBox/00%20Overview.md '/Documentation/Guide/UI_Components/CheckBox/') in the `show-order` item is checked. Both the `order` and `show-order` items are unbound items. 
