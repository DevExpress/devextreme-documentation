If you do not need a simple item to be bound to a [formData](/api-reference/10%20UI%20Components/dxForm/1%20Configuration/formData.md '/api-reference/10%20UI%20Components/dxForm '/Documentation/ApiReference/UI_Components/dxForm/'Configuration/#formData') field, create an unbound item. It can be useful if you need, for example, to hide or show some additional information. To create such an item, specify its [name](/api-reference/10%20UI%20Components/dxForm/5%20Item%20Types/SimpleItem/name.md '/api-reference/10%20UI%20Components/dxForm '/Documentation/ApiReference/UI_Components/dxForm/'Item_Types/SimpleItem/#name'), but do _not_ set its **dataField** property. You will be able to access the item by this name if you need to.

In the following example, the `order` item contains the [DataGrid](/concepts/05%20UI%20Components/DataGrid/00%20Getting%20Started%20with%20DataGrid '/Documentation/Guide/UI_Components/DataGrid/Getting_Started_with_DataGrid/') UI component. This UI component is shown only when the [CheckBox](/Documentation/Guide/UI_Components/CheckBox/) in the `show-order` item is checked. Both the `order` and `show-order` items are unbound items. 
