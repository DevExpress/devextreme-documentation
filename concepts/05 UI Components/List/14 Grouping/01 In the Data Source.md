Items in the **List** are grouped if they are grouped in the data source. The **List** recognizes a group when it encounters an object with the **key** and **items** fields. The **key** is the group header, the **items** are items that fell into the group. For example, the `fruitsVegetables` array from the following code produces two groups with three items each. Note that the **List** needs to be informed that it deals with grouped data, therefore its [grouped](/api-reference/10%20UI%20Components/dxList/1%20Configuration/grouped.md '/api-reference/10%20UI%20Components/dxList '/Documentation/ApiReference/UI_Components/dxList/'Configuration/#grouped') property is set to **true**.
...
#####See Also#####
- [List - Grouping | Customize Group Headers](/concepts/05%20UI%20Components/List/14%20Grouping/05%20Customize%20Group%20Headers.md '/Documentation/Guide/UI_Components/List/Grouping/Customize_Group_Headers/')
