...
In the following code, the **searchMode** property is set to *"contains"*. This mode allows users to find items that contain the search value. The **searchExpr** property is set to an array of two data fields, and the component searches them both. The **minSearchValue** property indicates that the search begins only when users type in two or more characters.
