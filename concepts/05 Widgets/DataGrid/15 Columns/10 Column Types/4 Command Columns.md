...
You can also customize command columns' cells using the [onCellPrepared](/api-reference/10%20UI%20Widgets/dxDataGrid/1%20Configuration/onCellPrepared.md '/Documentation/ApiReference/UI_Widgets/dxDataGrid/Configuration/#onCellPrepared') function. To distinguish between cells of a command column and other cells, check the argument's **column.command** field for the *"edit"*, *"adaptive"* or *"select"* value. In the following code, the editing column's cells are customized by attaching a new click handler to the *"Edit"* links: 
