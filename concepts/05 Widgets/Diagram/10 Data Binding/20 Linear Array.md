...
During the binding process, the widget creates a shape for every bound node and a connector between every pair of nodes linked by the _Key_ - _Parent Key_ relation. Note that edges are not maintained as entities in a data source, thus a detached connector disappears after rebinding.
