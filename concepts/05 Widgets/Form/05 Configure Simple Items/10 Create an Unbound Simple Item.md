...
In the following example, the `order` item contains the [DataGrid](/concepts/05%20Widgets/DataGrid/00%20Getting%20Started%20with%20DataGrid '/Documentation/Guide/Widgets/DataGrid/Getting_Started_with_DataGrid/') UI component. This UI component is shown only when the [CheckBox](/Documentation/Guide/Widgets/CheckBox/) in the `show-order` item is checked. Both the `order` and `show-order` items are unbound items. 
