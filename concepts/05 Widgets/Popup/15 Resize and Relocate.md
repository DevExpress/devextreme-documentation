...
By default, an end user is allowed to change the **Popup** position only on desktops. To enable this feature on other devices too, set the [dragEnabled](/api-reference/10%20UI%20Widgets/dxPopup/1%20Configuration/dragEnabled.md '/Documentation/ApiReference/UI_Widgets/dxPopup/Configuration/#dragEnabled') property to **true**. Note that the user drags the **Popup** by its title, so the title should _not_ be hidden.  
