...
Components that use the *controlled* state management mode delegate data storage and state management to other entities. Such components use *props* to receive data from their parents, and *callbacks* to notify their parent of state updates. The official React documentation recommendeds this approach for most users.
