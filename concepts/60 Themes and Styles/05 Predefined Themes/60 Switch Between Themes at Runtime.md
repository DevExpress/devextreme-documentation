...
1. Switch to a theme using the [DevExpress.ui.themes.current(themeName)](/api-reference/50%20Common/utils/ui/themes/3%20Methods/current(themeName).md '/Documentation/ApiReference/Common/utils/ui/themes/Methods/#currentthemeName') method. It accepts the `data-theme` attribute's value from the previous code. The following example shows how to apply the Generic Contrast theme:
