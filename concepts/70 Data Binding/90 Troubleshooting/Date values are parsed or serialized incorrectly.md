...
**Solution**:     
Different browsers parse date strings differently. To compensate for this inconsistency, DevExtreme components use an internal parser. It only supports date strings that comply with the <a href="https://en.wikipedia.org/wiki/ISO_8601" target="_blank">ISO 8601</a> standard, that is, strings that have the following formats:
...
Date strings in other formats are passed to the Date constructor. In some instances, this approach produces different results. Refer to the <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/parse" target="_blank">Date.parse()</a> article for more information. We encourage you to use the ISO 8601 formats to guarantee the absence of possible issues.
