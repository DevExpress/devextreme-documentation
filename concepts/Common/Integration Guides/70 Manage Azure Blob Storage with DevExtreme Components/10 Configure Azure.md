...
You need a Microsoft Account to set up Microsoft Azure. As of the writing of this article, Microsoft offers a free trial for most Azure products.
...
3. Follow the instructions to create a new Storage account.
    ...
    * Select a [redundancy policy](https://learn.microsoft.com/en-us/azure/storage/common/storage-redundancy) for your Azure storage. Azure can store copies of your data across multiple servers within the same data center, different data centers in the same region, or two different regions. You cannot manually select the secondary storage region — for maximum efficiency, the system automatically determines its location based on your primary region.
...
CORS policies give your Storage Blob an added layer of protection from unauthorized requests. Azure Storage Blobs don't have a CORS policy out of the box. Use the [service properties](https://learn.microsoft.com/en-us/rest/api/storageservices/set-blob-service-properties?tabs=microsoft-entra-id) of your blob to set its CORS policy.
