DevExtreme components function on the client side, which requires a license check during client-side code execution. Create a `ClientComponent` with the assigned license and run it in the `RootLayout` section before rendering any content.
